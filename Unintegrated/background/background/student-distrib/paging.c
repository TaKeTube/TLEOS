...
void paging_init()
{
    ...
    */
    ...
    set_fb_page(VBE_FB_ADDR);
    set_fb_page(FRAME_BUFFER_1_ADDR);
    set_fb_page(FRAME_BUFFER_2_ADDR);
    // set_background_page(BACKGROUND_ADDR);
}
...
void set_fb_page(uint32_t fb_addr)
{
    uint32_t index = fb_addr / PAGE_4MB_SIZE;
    
    page_directory[index].p = 1;
    page_directory[index].ps = 1;
    page_directory[index].g = 1;
    page_directory[index].base_addr = fb_addr >> MEM_OFFSET_BITS;

    page_directory[index+1].p = 1;
    page_directory[index+1].ps = 1;
    page_directory[index+1].g = 1;
    page_directory[index+1].base_addr = (fb_addr + PAGE_4MB_SIZE) >> MEM_OFFSET_BITS;
}

// void set_background_page(uint32_t bg_addr)
// {
//     uint32_t index = bg_addr / PAGE_4MB_SIZE;
    
//     page_directory[index].p = 1;
//     page_directory[index].ps = 1;
//     page_directory[index].g = 1;
//     page_directory[index].base_addr = bg_addr >> MEM_OFFSET_BITS;

//     page_directory[index+1].p = 1;
//     page_directory[index+1].ps = 1;
//     page_directory[index+1].g = 1;
//     page_directory[index+1].base_addr = (bg_addr + PAGE_4MB_SIZE) >> MEM_OFFSET_BITS;

//     page_directory[index+1].p = 1;
//     page_directory[index+1].ps = 1;
//     page_directory[index+1].g = 1;
//     page_directory[index+1].base_addr = (bg_addr + PAGE_4MB_SIZE*2) >> MEM_OFFSET_BITS;

//     page_directory[index+1].p = 1;
//     page_directory[index+1].ps = 1;
//     page_directory[index+1].g = 1;
//     page_directory[index+1].base_addr = (bg_addr + PAGE_4MB_SIZE*3) >> MEM_OFFSET_BITS;

//     page_directory[index+1].p = 1;
//     page_directory[index+1].ps = 1;
//     page_directory[index+1].g = 1;
//     page_directory[index+1].base_addr = (bg_addr + PAGE_4MB_SIZE*4) >> MEM_OFFSET_BITS;
// }
/*
*	flush_TLB
